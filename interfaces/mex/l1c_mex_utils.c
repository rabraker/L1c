...
void _mex_assert_scalar_struct(const mxArray** mxa, size_t idx) {
  if (mxGetM(mxa[idx]) != 1 || mxGetN(mxa[idx]) != 1) {
    mexErrMsgIdAndTxt("l1c:notScalar", "Argument %d must be a 1 by 1 struct.", idx + 1);
  }
  if (!mxIsStruct(mxa[idx])) {
    mexErrMsgIdAndTxt("l1c:notStruct", "Argument %d must be a struct.", idx + 1);
  }
}

void _mex_assert_double(const mxArray** mxa, size_t idx) {
  if (!mxIsDouble(mxa[idx])) {
    mexErrMsgIdAndTxt("l1c:notDouble", "Argument %d must be a double.", idx + 1);
  }
}

void _mex_assert_double_scalar(const mxArray** mxa, size_t idx) {
  if (!mxIsDouble(mxa[idx]) || !mxIsScalar(mxa[idx])) {
    mexErrMsgIdAndTxt(
        "l1c:notDoubleScalar", "Argument %d must be a scalar double.", idx + 1);
  }
}

void _mex_assert_num_outputs(int N, int N_required) {
  if (N < N_required) {
    mexErrMsgIdAndTxt("l1c:nlhs", "%d output(s) required.", N_required);
  }
}

void _mex_assert_num_inputs(int N, int N_required) {
  if (N < N_required) {
...
l1c_int _mex_get_int32_scalar_or_fail(const mxArray** mxa, size_t idx) {
  if (!mxIsInt32(mxa[idx]) || !mxIsScalar(mxa[idx])) {
    mexErrMsgIdAndTxt(
        "l1c:notInt32Scalar", "Argument %d must be a scalar double.", idx + 1);
  }
...
void _mex_assert_1darray(const mxArray** mxa, size_t idx) {
  ...
  if (d > 2 || ((n != 1) && (m != 1))) {
    mexErrMsgIdAndTxt(
        "l1c:not1DArray", "Input %d must have only 1 dimension.", idx + 1);
  }
}

void _mex_assert_2Darray_with_size(const mxArray** mxa,
                                   size_t idx,
                                   l1c_int n,
                                   l1c_int m) {
  /* Matlab stores as column major. But the call M=number of rows and N=number of
   columns for some bizzar reason. */
  ...
  if (d > 2) {
    mexErrMsgIdAndTxt(
        "l1c:not2DArray", "Input %d must have only 1 dimension.", idx + 1);
  }
  if (n_act < n_sizet || m_act < m_sizet) {
    mexErrMsgIdAndTxt("l1c:ArraySize",
                      "Input %d has size (%d, %d); expected at least (%d, %d)\n",
                      idx + 1,
                      n_act,
                      m_act,
                      n,
...
void _mex_get_double_array_or_fail(const mxArray** mxa,
                                   size_t idx,
                                   double* x[],
                                   l1c_int* N) {
  size_t n = mxGetN(mxa[idx]);
  ...
  nm = n * m;
  if (nm > INT_MAX) {
    mexErrMsgIdAndTxt("l1c:ArrayTooLarge", "L1c can only index up to %d\n", INT_MAX);
  }
...
void _mex_get_int_array_or_fail(const mxArray** mxa, size_t idx, int* x[], l1c_int* N) {
  size_t n = mxGetN(mxa[idx]);
  size_t m = mxGetM(mxa[idx]);
  size_t nm = 0;
