...
function [x, u, niter] = l1qc_newton_fcns(x0, u0, A, At, b, epsilon, ...
                                       tau, newtontol, newtonmaxiter, ...
  ...
  for niter=1:newtonmaxiter
    ...
    if niter == 5
      jopts.FileName = 'test_data/descent_data.json';
    ...
    % struct('atr', atr, 'ntgu', ntgu, 'sig11', sig11, 'sig12', sig12, 'w1p', w1p); 
    if (cgres > 1/2)
    ...
    if niter == 5
      jopts.FileName =  'test_data/max_stepsize_data.json';
    ...
    if niter == 5
      jopts.FileName = 'test_data/line_search_data.json';
