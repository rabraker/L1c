...
\end{equation}
using a log-barrier algorithm, which is based on the implementation in [[https://statweb.stanford.edu/~candes/l1magic/][l1-magic]],  [[sec:l1qc_mod][with a few modifications]]. Currently, the code *only* operates in large-scale mode, which means that the transformation $Ax$ (and the adjoint, $A^Ty$) are performed by user supplied functions, rather than an explicit matrix-vector multiplication. This also means that solving for the descent direction is always done via conjugate gradients.

- Solving the Anistropic Total Variation (TV) denoising using  [[https://www.ece.rice.edu/~tag7/Tom_Goldstein/Split_Bregman.html][Bregman Splitting]]. Given a noisy $n$ by $m$ image $f$, this solves the problem
  \begin{equation}
    \min_{u} ||\nabla_{x}||_{1} + ||\nabla_{y}||_{1} + ||u-f||_{2}.
  \end{equation}
...
*** Unit Tests
The unit tests depend python3  with numpy+scipy and two additional libraries:
1. [[https://github.com/libcheck/check][libcheck]] is the unit testing framework. 
2. cJSON

Almost all of the test data is generated in python and saved as json files in ~test/test_data/~.
...
./configure --with-fftw3 --with-fftw3-threads=omp --enable-mex \   --with-mex-prefix=/home/arnold/matlab/l1c
#+END_SRC
...
If you compile with FFTW+OpenBlas, it is important that both libraries are compiled with openmp. I don't quite understand what happens, but if this is not the case, performance can suffer dramatically. 
...
3. We need different prefix targets for the shared library and the interfaces modules.
