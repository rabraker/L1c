...
static void setup(char *fname){
  ...
  setup_status +=extract_json_double_array(test_data_json, "MtEt_EMx", &dctd->MtEty_EMx_exp, &dctd->mtot);
...
static void setup_square(void){
  setup("dct2_small.json");
}

static void setup_tall(void){
  setup("dct2_small_tall.json");
}


static void setup_wide(void){
  setup( "dct2_small_wide.json");
}


static void setup_pure_square(void){
  setup("dct2_small_pure_dct.json");
}
