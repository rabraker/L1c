...
static void setup(char *fname){
  ...
  setup_status +=extract_json_double_array(test_data_json, "MtEt_EMx", &dctd->MtEty_EMx_exp, &dctd->m);
...
static void setup_small(void){
  setup("dct_small.json");
}


static void setup_large(void){
  setup( "dct_large.json");
}


static void setup_pure(void){
  setup( "dct_small_pure_dct.json");
}
