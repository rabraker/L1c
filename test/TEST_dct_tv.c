...
static void setup(char* fname, BpMode bp_mode, DctMode dct_mode) {
  ...
  int status_setup = l1c_setup_dctTV_transforms(dctd->n,
                                                dctd->mrow,
                                                dctd->mcol,
                                                dctd->alp_v,
                                                dctd->alp_h,
...
START_TEST(test_Wz_synth) {
  ...
  ck_assert_double_array_eq_tol(dctd->mtot, dctd->z_in, dctd->Mx_act, TOL_DOUBLE_SUPER);
}
END_TEST
...
START_TEST(test_Rx) {
  ...
  ck_assert_double_array_eq_tol(dctd->n, dctd->Rx_exp, dctd->Rx_act, TOL_DOUBLE_SUPER);
}
END_TEST
