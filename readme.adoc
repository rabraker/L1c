...
./configure [--with-fftw3-threads=[ARG]] [--with-mkl] [--enable-mex] \
...
* Two test cases still rely on matlab to produce the required data. If matlab is unavailible, you can run the tests anyway but `test_line_search` and `test_l1qc_newton_1iter` will fail. Alternatively, the data can be https://rabraker.com/l1c_data/l1c_extra_data.tar[downloaded] and should be unpacked into `test/test_data`.
